...
`vendor/attacore` 是上游的仓（`D61`；2026-09-05 起钉在没打 tag 的 `50e153e` 上，见 `D65`）。**改那边的代码是明令禁止的**——
...
**隔离是部署侧的事**（`D62`，2026-09-02）。`EXE-029` 的四个容器后端**不实现**，
`D51` 由它取代，`Q24` 随之关闭。启动横幅上那八条是**交给部署方的清单**，
不是我们的待办——**它不会因为我们做点什么而缩短**。

**交给部署方的那些写在 [docs/deployment.md](docs/deployment.md) §2**，
不只是横幅上一行——`D64` 的资源上限也在那里（硬上限进程内做不到，
`setrlimit` 是进程级的且要 `unsafe`）。

没有容器之后，边界由这几样兑现（都可跑可测）：工具集只有
`Read`/`Glob`/`Grep`/`EmitRow` · 路径锁在只读工作区里（没声明代码仓就一个文件都不准碰）·
派工单不含凭据也不含到 XOps 的网络路径 · 唯一的写向只通向已声明的表。
**剩下的主要风险是 `EXE-008` 资源上限**——一个跑飞的技能能吃满这台机器。

## 从外面跑一遍

`scenarios/` 是**用使用者的方式、全程经 MCP** 走的三条路（任务 · 代码仓 · 流程）。
它挡的是一类具体的缺陷:**单元全绿、装配也过，但那条链在运行时是断的**——
到今天为止已经用这个办法撞出十二处，共同点是**不报错**。

```bash
scenarios/run.sh          # 要一份 .env，里面有真的模型 key
```

改了装配层、执行链或任何一处注入位，跑它一遍再提交。
