...
**三面已经接上了**（2026-09-05）：`api:mcp.tool.*` · `api:http.paths.*` ·
`sql:*` 的四个前缀，来源都是 `xopsd --dump-contracts`——
**问的是装配好的进程，不是源码**。
...
⚠️ **`sql:*` 只自证得了四个前缀**：`sql:table.kv` · `sql:relation.` ·
`sql:meta.column-type.` · `sql:meta.auto-column.`。
逻辑系统表（`sql:table._runs` 这些）、键的布局（`sql:layout.*`）、
投影与命名规则**是约定，不是能从库里读出来的东西**——硬凑一个"自证"出来，
只会制造一堆假的漂移。**认领不了的那些 `check` 会逐条报出来**：
不自证是一回事，**悄悄地不自证是另一回事**。

它第一次跑找出 **22 条没被逐列登记的关系投影列**——基线以前只在正文里
用散文写过"列：`row` · `user` · …"，而**散文比不出漂移**。
⚠️ 这一面比 `api:*` 那一面更要紧：**数据是唯一不能回滚的东西。**

还欠一面：`rust:*`（`cargo-public-api` 快照）。⚠️ **建议只做告警不做硬失败**——
`rust.md` 现在记的是**行为约定**（"⚠️ 这条线在 `D61` 时断过"那种），
而快照给的是签名。硬比会逼着把散文换成清单，**那是拿一份有用的东西
换一份机械的东西**。
