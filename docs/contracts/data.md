...
### Element: sql:table._notices
- module: xops-table
- 每条通知一行，属于一个用户（`TBL-010`）。**平台全局表**——`project` 是一列而不是表的归属，
  跨项目聚合靠的就是这一点（`NTF-014`）。
- 列：`notice` · `user` · `project` · `kind` · `subject` · `text` · `createdAt` · `readAt`。
- ⚠️ **它是行级可见性的唯一例外**（`NTF-010`、`TBL-024`）：读写被平台硬限定为
  `user = 令牌持有人`。立这个例外而不是做通用行级权限，理由是它
  **只有一张表、只有一条规则、平台自己写死**。
  **任何"给别的表也加一点行级可见性"的请求都不在这个例外的射程里。**
- ⚠️ 它也**不参与表专属 tool 的派发、不允许建自由看板**（`TBL-011`、`BRD-004`）。

### Element: sql:table._notices.column.readAt
- module: xops-notice
- **全系统唯一一个用户可改的系统表列**（`NTF-011`）。三条边界各自的落法：
  只能改自己那一行（比一次 `notice.user == actor`，不一致就当作不存在）·
