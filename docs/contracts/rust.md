...
- ⚠️ **这一道是事后归类；"撞上即终止"在引擎那一侧**
  （`EmbeddedEngine` 把 `max_budget_tokens` 按派工单设，引擎每次调用后比一次）。
  两处都要有：**引擎那一道管住烧，这一道管住算**——桩引擎、以及任何不自己收手的
  引擎，都靠这一道兜住。
- ⚠️ **比的是 `>=` 不是 `>`。** `TSK-005` 原话是"消耗的模型 token **撞上**这个数"，
  引擎那一侧也是 `>=`。写成 `>` 的话，一次**正好被引擎按预算掐掉**的执行
  在这里会被判成成功——引擎停在 `total == budget`，而 `>` 不成立。
  **那个缝很窄，而且只在缓存用量为零时才露出来**，不会自己在测试里出现。
...
- module: xopsd
- consumers: [部署]
- 交出去的两个服务面，外加**三处不能悄悄发生的降级**，与**一张实际拦着的界**：
...
- **新增 `bounds`**（`D64`）：`EXE-008` 的**硬上限**（OS 级 CPU / 内存 / 磁盘）
  在进程内做不到，归部署侧——但实际的跑飞路径已经被六道数盖住，逐条上横幅。
  ⚠️ **它与 `unsatisfied` 是一对，两张表要挨着放**：只说没兑现的，
  读的人会以为完全敞着；只说这一张，**就是在假装 `EXE-008` 做完了**。
...
- module: xops-exec
- consumers: [xopsd]
- **AttaCore 以库的形式嵌进来，一个进程**（`D61`）。源码是 git 子模块，**钉在 `50e153e`**——⚠️ **那是一个没打 tag 的提交**（`D65`）。
...
- **单次 token 上限按派工单设进 `Settings.execution.max_budget_tokens`**（`TSK-005`）。
  引擎默认的 `EngineBudget` 读它，在**每次模型调用之后**比一次：到九成发一句提醒，
  撞上就**结束这个回合**，`stop_reason` 是 `budget_exceeded`。
  ⚠️ **这才是"撞上即强制终止"那一半**——`rust:xops-exec#Runtime` 那一道是事后归类，
  它决定这次执行算不算数，**而那时钱已经花了**。两处都要有：
  **引擎这一侧管住烧，`Runtime` 那一侧管住算。**
- **接的是自己的 `BudgetPolicy`（`RunBudget`），不是引擎默认的那个。**
  ⚠️ 默认的 `EngineBudget` 读 `Spend::total_tokens()`——**input + output，
  把缓存放在外面**；而我们这一侧四项全加。两个数不同就意味着
  **"引擎在哪一刻收手"与"这次执行算不算数"是按两把尺子判的**。
- ⚠️ **上游把这个选择从引擎内部的一次加法搬到了策略层**（`D65` 跟的那一版）：
  `Spend` 分开带四个数、命名了两种读法，`EngineBudget` 继续读 `total_tokens()`
  所以 `max_budget_tokens` 含义不变，**想把缓存算进去的部署自己实现一个**。
  我们因此读 `all_tokens()`——**两边读同一个数，口径合一**。
- 我们选四项全加的理由：`input_tokens` **不含命中缓存的部分**，
  而 `SkillScene` 的系统提示是 `system_cached` 发出去的——只加 input + output，
  少算会从缓存那道门原样回来。`TSK-005` 比的是 **token 上限不是账单**，
  两者之间宁可高估。
- `max_budget_tokens` 仍然照设，**但那只是兜底**：万一策略没接上仍有一个上限，
  **而不是让两个上限并存**。
- ⚠️ **有测试盯着它真的收手**：脚本里排两趟、预算只够第一趟，
  断言**第二趟发都没发出去**。拆掉这根线它当场红（验过）。
...
### Element: rust:xops-exec#IsolationLevel::bounds
- module: xops-exec
- consumers: [xopsd, 部署]
- **实际在拦着的那几道**（`D64`）：`TSK-005` 单次 token 上限 · `EXE-019` 看门狗超时 ·
  `EXE-027` 并发上限 · 上游的工具回话上界 · `EXE-025` 产物截断 · `EXE-031` 产出行上限。
- ⚠️ **它存在的理由是「只说没兑现的那张表会误导人」。** `EXE-008` 的硬上限确实没有，
  而且在进程内做不到（`setrlimit` 是进程级的且要 `unsafe`，本仓 forbid；
  cgroups 要 Linux 加权限；线程没有内存上限这回事）。
  但**实际的跑飞路径已经被这几道盖住**，而且每一道都可测。
- ⚠️ **一条都不能少，有测试盯着**：少一条不是少一行文字，**是少一道界**，
